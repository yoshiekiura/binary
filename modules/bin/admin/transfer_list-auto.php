...
$form = _lib('pea', 'bin');
$form->initRoll("WHERE `balance`{$separator}{$min_transfer} AND `active`=1 ORDER BY `balance` DESC");
...
$form->roll->input->balance->setTitle('Amount');
...
if (!empty($_POST['transfer']))
{
	...
	if ($_POST['transfer'] == 'download')
	{
		...
		if ($is_surcharge)
		{
			$q = "SELECT b.`id` AS `memberID`, a.`params`, b.`username`, b.`name` AS `nama`, b.`bank_name` AS `bank`, b.`bank_no` AS `no_rek`, b.`balance` AS `total`, '0' AS `potongan`, '0' AS `transfer`, '0' AS `done` FROM `bin` AS b LEFT JOIN `bbc_account` AS a ON (b.`user_id`=a.`user_id`) WHERE b.`balance` {$separator} {$min_transfer} AND b.`active`=1 ORDER BY b.`balance` DESC";
		}else{
			$q = "SELECT `id` AS `memberID`, `username`, `name` AS `nama`, `bank_name` AS `bank`, `bank_no` AS `no_rek`, `balance` AS `total`, '0' AS `done` FROM `bin` WHERE `balance` {$separator} {$min_transfer} ORDER BY `balance` DESC";
		}
		$r = $db->getAll($q);
		if (!empty($r))
		{
			if ($is_surcharge)
			{
				foreach ($r as &$d)
				{
					$params = config_decode($d['params']);
					$charge = !empty($params['NPWP']) ? 'surcharge_npwp' : 'surcharge_npwp_no';
					unset($d['params']);
					$d['potongan'] = bin_charge($d['total'], $config['surcharge']);
					$d['potongan']+= bin_charge($d['total']-$d['potongan'], $config[$charge]);
					$d['transfer'] = $d['total'] - $d['potongan'];
				}
			}
